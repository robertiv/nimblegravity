...
export default function InfoEmpleos({ empleo, candidato }: InfoEmpleosProps) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    console.log("Enviando postulación con los siguientes datos:");
    console.log("UUID del candidato:", candidato.uuid);
    console.log("ID del empleo:", empleo.id);
    console.log("ID del candidato:", candidato.candidateId);
    console.log("URL del repositorio:", repoUrl.trim());
    // try {
    ...
    <div className="border border-zinc-200 rounded-md p-4 bg-zinc-50">
      <div className="flex items-start justify-between gap-4 mb-3">
        <div>
          ...
          <h4 className="text-sm font-semibold text-zinc-800">{empleo.title}</h4>
        </div>
        {success && (
